...
export default function ChatWidget({ venueId, venueName = "MuseBot" }) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          showBookingForm: data.show_booking_form,
        },
      ]);
  ...
  const submitGroupBooking = async () => {
    const exhibit = formExhibits.find((e) => e._id === formData.exhibitId);
    if (!exhibit || !formData.date || !groupData.groupName.trim()) return;

    const slotsRes = await fetch(
      `${API_BASE}/slots/?exhibit_id=${exhibit._id}&date=${formData.date}`,
    );
    ...
    if (!slots.length) {
      setMessages((prev) => [
        ...
        {
          sender: "bot",
          text: `No slots found for ${exhibit.name} on ${formData.date}.`,
        },
      ]);
      return;
    }
    ...
    setMessages((prev) => [
      ...
      {
        sender: "user",
        text: `Group booking: ${groupData.groupName} (${groupData.headcount} people) for ${exhibit.name} on ${formData.date}`,
      },
    ]);
    ...
    try {
      ...
      if (!res.ok) {
        setMessages((prev) => [
          ...prev,
          { sender: "bot", text: data.detail || "Group booking failed." },
        ]);
        return;
      }
      ...
      setMessages((prev) => [
        ...
        {
          sender: "bot",
          text: `Group order created for ${data.group_name}: ₹${data.pricing.final_amount} (${data.pricing.discount_percent}% off for ${data.pricing.headcount} people). Complete payment in the popup...`,
        },
      ]);
      ...
      setMessages((prev) => [
        ...prev,
        { sender: "bot", text: "Something went wrong with group booking." },
      ]);
    }
  };
              ...
              {m.showBookingForm && formExhibits.length > 0 && (
                <div className="mt-3 bg-white border rounded-lg p-3">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, exhibitId: e.target.value })
                    }
                    className="w-full border rounded-lg px-2 py-2 mb-3 text-sm"
                  >
                    {formExhibits.map((ex) => (
                      <option key={ex._id} value={ex._id}>
                        {ex.name} — ₹{ex.basePrice}
                      </option>
                    ))}
                  </select>
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        <button
                          onClick={() =>
                            setGroupData({
                              ...groupData,
                              headcount: Math.max(10, groupData.headcount - 5),
                            })
                          }
                        ...
                        <button
                          onClick={() =>
                            setGroupData({
                              ...groupData,
                              headcount: groupData.headcount + 5,
                            })
                          }
                          className="w-8 h-8 bg-gray-200 rounded-full font-bold"
                        >
                  ...
                  <button
                    onClick={
                      isGroupBooking ? submitGroupBooking : submitBookingForm
                    }
